#Maybe, make sure process can only be ran via a schedule if time is between 5am and 12 am est.
#Then, add dimension tables.
...
START_DATE = datetime.strptime("2025-03-27", "%Y-%m-%d") # 03-20 then 03-28 then #07-19
END_DATE = datetime.strptime("2025-06-08", "%Y-%m-%d") # 03-21 then 07-14 then #09-30
...
for date in date_range:
    ...
    run_r_script >> run_add_people_to_staging >> run_update_people_dim >> run_insert_people_dim_data >> run_add_team_to_staging >> run_update_team_dim >> run_insert_team_dim_data >> run_dbt >> run_add_on_surrogate_keys_to_fact >> run_append_stats >> run_add_surrogate_keys_to_team_games_fact >> run_append_team_game_stats
